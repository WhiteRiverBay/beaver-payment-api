package ltd.wrb.payment.util.crypto;
...
import ltd.wrb.payment.util.crypto.common.BIUtil;
import ltd.wrb.payment.util.crypto.common.ByteUtil;
import ltd.wrb.payment.util.crypto.common.Hash;
import ltd.wrb.payment.util.crypto.cryptohash.Keccak256;
import ltd.wrb.payment.util.crypto.jce.ECKeyAgreement;
import ltd.wrb.payment.util.crypto.jce.ECKeyFactory;
import ltd.wrb.payment.util.crypto.jce.ECKeyPairGenerator;
import ltd.wrb.payment.util.crypto.jce.ECSignatureFactory;
import ltd.wrb.payment.util.crypto.jce.TronCastleProvider;
...
public class ECKey implements Serializable { // SignInterface {
     ...
     * signature. ECDSA
     * signatures are mutable in the sense that for a given (R, S) pair, then both
     * (R, S) and (R, N -
     * S mod N) are valid signatures. Canonical signatures are those where 1 <= S <=
     * N/2
     *
     * <p>
     ...
     * things up when you
     * know you have the right values already. The compression state of pub will be
     * preserved.
     *
     * @param priv -
     * @param pub  -
     ...
     * multiplying the generator point by the private key. This is used to speed
     * things up when you
     * know you have the right values already. The compression state of the point
     * will be preserved.
     *
     * @param priv -
     * @param pub  -
     ...
     * allKeys is the
     * correct one. Because the key recovery operation yields multiple potential
     * allKeys, the correct
     * key must either be stored alongside the signature, or you must be willing to
     * try each recId in
     ...
     * loop from 0 to 3,
     * and if the output is null OR a key that is not the one you expect, you try
     * again with the next
     * recId.
     *
    ...
    // uncompressed form.
    // * Normally you would never need this: it's for specialised scenarios or when
    // backwards
