...
public class ByteUtil {
  ...
  /**
   * Turn nibbles to a pretty looking output string Example. [ 1, 2, 3, 4, 5 ]
   * becomes
   * '\x11\x23\x45'
   *
  ...
  /**
   * Cast hex encoded value from byte[] to int Limited to Integer.MAX_VALUE:
   * 2^32-1 (4 bytes)
   *
   ...
   * array. If {@code
   * input} has not enough bytes return array will be right padded with zero
   * bytes. I.e. if {@code
   * offset} is higher than {@code input.length} then zero byte array of length
   * {@code len} will be
   * returned
   */
   ...
   * {@link #parseBytes(byte[], int, int)} method, thus,
   * result will be right-padded with zero bytes if there is not enough bytes in
   * {@code input}
   *
