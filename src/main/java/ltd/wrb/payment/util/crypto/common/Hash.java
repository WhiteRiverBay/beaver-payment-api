...
import static ltd.wrb.payment.util.crypto.common.ByteUtil.EMPTY_BYTE_ARRAY;
import static ltd.wrb.payment.util.crypto.common.ByteUtil.isNullOrZeroArray;
import static ltd.wrb.payment.util.crypto.common.ByteUtil.isSingleZero;
...
public class Hash {
   ...
   * byte with value
   * 0x80 plus the length of the string followed by the string. The range of the
   * first byte is thus
   * [0x80, 0xb7].
   */
   ...
   * single byte with
   * value 0xb7 plus the length of the length of the string in binary form,
   * followed by the length
   * of the string, followed by the string. For example, a length-1024 string
   * would be encoded as
  ...
  /**
   * Reason for threshold according to Vitalik Buterin: - 56 bytes maximizes the
   * benefit of both
   * options - if we went with 60 then we would have only had 4 slots for long
   * strings so RLP would
   * not have been able to store objects above 4gb - if we went with 48 then RLP
   * would be fine for
